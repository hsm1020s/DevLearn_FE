...
| 프로젝트 기간 | 2026.04.20 ~ 진행 중 (달력 기준) |
| 실 투입 공수 | **11 공수** (2026.05.13 기준) |
| Back-End | Java 21, Spring Boot 4.0.5, MyBatis 3, PostgreSQL 17, pgvector, Gradle 9.4.1 |
| Front-End | React 19, Vite 6, Zustand 5, Tailwind CSS 3, React Router 6, ReactFlow 11, framer-motion, recharts, react-markdown, html-to-image + jsPDF |
| LLM (클라우드) | OpenAI GPT-5.4 mini, Claude Sonnet 4.6 |
| LLM (로컬) | Llama 3.1 8B, EXAONE 3.5 32B, GPT-OSS 20B (Ollama 서버) |
...
클라이언트(React) → Spring Boot 필터/컨트롤러/서비스 → 외부 LLM · 미디어 도구 · PostgreSQL+pgvector 의 흐름.

![System Architecture](docs/architecture/system-architecture.svg)

### 파인만 학습 파이프라인

PDF 업로드부터 RAG 기반 채점까지 3단계: **Ingestion**(PDFBox → 목차 인식 → 청크 분할 → Ollama 임베딩 768-dim) → **Synthesis**(LLM 2-pass 로 챕터 질문 + 마인드맵 자동 생성) → **학습/채점**(pgvector 코사인 검색 → AnswerGraderService → 누락 개념 + 출처 패널).

![Feynman Pipeline](docs/architecture/feynman-pipeline.svg)

### AI 강의 영상 생성 파이프라인

챕터 단위 `batch_id` 발급 후 **Phase 1 Script**(LLM 대본) → **Phase 2 Audio**(OpenAI TTS-1 슬라이드별 MP3) / **Phase 3 Slides**(LLM HTML → Playwright 1920×1080 캡처) → **Phase 4 Video**(FFmpeg 로 오디오 길이 기반 슬라이드 duration 합성 + VTT 자막). 각 단계는 `lecture_*_runs` 테이블에 독립 기록되어 실패 지점부터 재실행 가능.

![Lecture Pipeline](docs/architecture/lecture-pipeline.svg)

> 📁 다이어그램 소스(.mmd) 및 PNG 버전: [docs/architecture/](docs/architecture/)
