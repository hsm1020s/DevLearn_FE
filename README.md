...
| 항&#8288;목 | 내용 |
|------|------|
| 프로젝트&nbsp;이&#8288;름 | DevLearn (데브런) |
| 프로젝트&nbsp;기&#8288;간 | 2026.04.20 ~ 진행 중 (달력 기준) |
| 실&nbsp;투&#8288;입&nbsp;공&#8288;수 | **14 공수** (2026.05.16 기준, 10커밋 이상 작업일 기준) |
| Back-End | Java 21, Spring Boot 4.0.5, MyBatis 3, PostgreSQL 17, pgvector, Gradle 9.4.1 |
| Front-End | React |
| LLM&nbsp;(클&#8288;라&#8288;우&#8288;드) | OpenAI GPT-5.4 mini, Claude Sonnet 4.6 |
| LLM&nbsp;(로&#8288;컬) | 맥북 m5 pro RAM 64GB, GPT-OSS 20B, EXAONE 3.5 32B, Llama 3.1 8B |
...
**25개 테이블, 19개 컨트롤러, 83개 API 엔드포인트** (2026-05-16 기준)
...
- **파이프라인 4단계**: 텍스트 추출(PDFBox) → 목차 인식 → 챕터 분류 → 벡터 임베딩(OpenAI `text-embedding-3-small`, 768-dim 축소 후 pgvector 저장).
- **파이프라인은 클라우드 LLM 전용**: 질문 합성 · 답변 채점 · 마인드맵 합성 등 파이프라인 단계는 안정성을 위해 클라우드 모델로 통일. 로컬 LLM은 채팅에서만 선택 가능.
...
flowchart LR
    ...
    I2 --> I3["OpenAI 임베딩<br/>text-embedding-3-small · 768-dim"]
    I3 --> V[("pgvector")]
